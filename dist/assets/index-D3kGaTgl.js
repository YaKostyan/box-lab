(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))n(s);new MutationObserver(s=>{for(const i of s)if(i.type==="childList")for(const r of i.addedNodes)r.tagName==="LINK"&&r.rel==="modulepreload"&&n(r)}).observe(document,{childList:!0,subtree:!0});function a(s){const i={};return s.integrity&&(i.integrity=s.integrity),s.referrerPolicy&&(i.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?i.credentials="include":s.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function n(s){if(s.ep)return;s.ep=!0;const i=a(s);fetch(s.href,i)}})();const Me={},jt="toffipacks-api-session-v1",Ce=Me,ge=String(Ce?.VITE_API_BASE_URL??"").trim().replace(/\/$/,""),k=!!ge;class Dt extends Error{status;code;constructor(e,a,n="request_error"){super(a),this.status=e,this.code=n}}function lt(){localStorage.removeItem(jt)}function xt(){return!!localStorage.getItem(jt)}async function I(t,e={}){if(!k)throw new Dt(0,"Backend API is not configured.","api_disabled");const a=localStorage.getItem(jt),n=await fetch(`${ge}${t}`,{...e,headers:{Accept:"application/json",...e.body?{"Content-Type":"application/json"}:{},...a?{Authorization:`Bearer ${a}`}:{},...e.headers}}),s=await n.json().catch(()=>({}));if(!n.ok)throw n.status===401&&lt(),new Dt(n.status,s.error?.message??"Сервер не зміг виконати запит.",s.error?.code);return s}async function re(t,e){const a=await I(t,{method:"POST",body:JSON.stringify(e)});return localStorage.setItem(jt,a.token),a.account}const q={products:async()=>(await I("/api/products")).products,login:(t,e)=>re("/api/auth/login",{phone:t,password:e}),register:t=>re("/api/auth/register",t),me:async()=>(await I("/api/auth/me")).account,updateMe:async t=>(await I("/api/auth/me",{method:"PATCH",body:JSON.stringify(t)})).account,logout:async()=>{try{await I("/api/auth/logout",{method:"POST"})}finally{lt()}},createOrder:async t=>(await I("/api/orders",{method:"POST",body:JSON.stringify(t)})).order,myOrders:async()=>(await I("/api/me/orders")).orders,adminProducts:async()=>(await I("/api/admin/products")).products,createProduct:async t=>(await I("/api/admin/products",{method:"POST",body:JSON.stringify(t)})).product,updateProduct:async(t,e)=>(await I(`/api/admin/products/${encodeURIComponent(t)}`,{method:"PATCH",body:JSON.stringify(e)})).product,deleteProduct:t=>I(`/api/admin/products/${encodeURIComponent(t)}`,{method:"DELETE"}),resetProducts:async()=>(await I("/api/admin/products/reset",{method:"POST"})).products,adminOrders:async()=>(await I("/api/admin/orders")).orders,updateOrder:async(t,e)=>(await I(`/api/admin/orders/${encodeURIComponent(t)}`,{method:"PATCH",body:JSON.stringify(e)})).order,deleteOrder:t=>I(`/api/admin/orders/${encodeURIComponent(t)}`,{method:"DELETE"}),adminClients:async()=>(await I("/api/admin/clients")).clients,updateClient:async(t,e)=>(await I(`/api/admin/clients/${encodeURIComponent(t)}`,{method:"PATCH",body:JSON.stringify(e)})).client,backup:()=>I("/api/admin/backup")},At=5e4,Q=1e3,Ie=2,Ee=1,Zt=.5,te=[{id:"box-301",number:"301",name:"Самозбірна коробка №301",dimensions:{length:165,width:100,height:30},basePrice:5,sourceQuantity:1e3},{id:"box-302",number:"302",name:"Самозбірна коробка №302",dimensions:{length:130,width:130,height:50},basePrice:7,sourceQuantity:800},{id:"box-303",number:"303",name:"Самозбірна коробка №303",dimensions:{length:190,width:150,height:100},basePrice:17,sourceQuantity:800},{id:"box-304",number:"304",name:"Самозбірна коробка №304",dimensions:{length:230,width:150,height:35},basePrice:11,sourceQuantity:800},{id:"box-305",number:"305",name:"Самозбірна коробка №305",dimensions:{length:160,width:85,height:110},basePrice:6.5},{id:"box-306",number:"306",name:"Самозбірна коробка №306",dimensions:{length:145,width:145,height:40},basePrice:6.5,sourceQuantity:1e3},{id:"box-307",number:"307",name:"Самозбірна коробка №307",dimensions:{length:280,width:180,height:100},basePrice:18,sourceQuantity:500},{id:"box-308",number:"308",name:"Самозбірна коробка №308",dimensions:{length:250,width:180,height:40},basePrice:12,sourceQuantity:800},{id:"box-309",number:"309",name:"Самозбірна коробка №309",dimensions:{length:210,width:150,height:50},basePrice:11,sourceQuantity:900},{id:"box-310",number:"310",name:"Самозбірна коробка №310",dimensions:{length:260,width:260,height:50},basePrice:15,sourceQuantity:600},{id:"box-311",number:"311",name:"Самозбірна коробка №311",dimensions:{length:370,width:170,height:100},basePrice:17},{id:"box-101",number:"101",name:"Самозбірна коробка №101",dimensions:{length:178,width:115,height:48},basePrice:4,sourceQuantity:800}],De=[{question:"Як відбувається доставка?",answer:"Доставляємо по Києву та Київській області. Формат, точну адресу й вартість потрібно уточнити з менеджером під час підтвердження заявки."},{question:"Які строки виготовлення?",answer:"Строк залежить від розміру коробки, тиражу та завантаження виробництва. Менеджер називає точну дату до запуску замовлення."},{question:"Що отримує постійний клієнт?",answer:"Після підтвердження менеджером у кабінеті активується персональна ціна. Вона автоматично відображається в каталозі, калькуляторі та кошику."},{question:"Як проходить оплата?",answer:"Форму оплати, рахунок і підсумкову суму менеджер погоджує з вами до початку виготовлення."},{question:"Чи працюєте ви з малим і великим бізнесом?",answer:"Так. Можна почати з невеликої партії або замовити регулярний великий тираж. Калькулятор рахує до 50 000 коробок, більший обсяг прораховує менеджер."},{question:"Чи робите коробки під індивідуальний запит?",answer:"Так. Якщо серед готових розмірів немає потрібного, вкажіть габарити й особливості замовлення в коментарі. Менеджер уточнить деталі та підготує розрахунок."}];function $(t){return new Intl.NumberFormat("uk-UA",{style:"currency",currency:"UAH",minimumFractionDigits:Number.isInteger(t)?0:2,maximumFractionDigits:2}).format(t)}function D(t,e){return t.basePrice+(e>=Q?Ee:Ie)}function it(t,e,a){return a?.partner?t.basePrice+Math.min(Math.max(a.fixedMarkup,0),.99):D(t,e)}function Ot(t){const{length:e,width:a,height:n}=t.dimensions;return e*a*n}function Bt(t,e,a=0){const n=[t.length,t.width,t.height].sort((o,c)=>c-o),s=[e.length,e.width,e.height].sort((o,c)=>c-o),i=n.map((o,c)=>(s[c]-o)/2),r=i.map(o=>Math.max(0,a-o));return{fits:r.every(o=>o===0),clearancesPerSide:i,deficitsPerSide:r}}const u={accounts:"toffipacks-accounts-v3",orders:"toffipacks-orders-v3",session:"toffipacks-session-v3",cart:"toffipacks-cart-v1",products:"toffipacks-products-v1",fit:"toffipacks-fit-v1",measurements:"toffipacks-measurements-v1"},he=/^[\p{L}\p{N}._-]+$/u,kt=new Date().toISOString(),be=[{id:"account-admin",name:"Адміністратор ToffiPacks",phone:"+380000000001",company:"ToffiPacks",password:"admin123",role:"admin",partner:!1,fixedMarkup:Zt,createdAt:kt},{id:"account-partner",name:"Постійний клієнт",phone:"+380671112233",company:"",password:"client123",role:"client",partner:!0,fixedMarkup:Zt,createdAt:kt}],ve=[];function mt(t,e){try{const a=localStorage.getItem(t);return a?JSON.parse(a):e}catch{return e}}function h(t,e){localStorage.setItem(t,JSON.stringify(e))}function Oe(){localStorage.getItem(u.accounts)||h(u.accounts,be),localStorage.getItem(u.orders)||h(u.orders,ve),localStorage.getItem(u.cart)||h(u.cart,[]),localStorage.getItem(u.products)||h(u.products,te.map(t=>({...t,active:!0,updatedAt:kt})))}Oe();k&&(h(u.accounts,[]),h(u.orders,[]),xt()||localStorage.removeItem(u.session));const ye=mt(u.fit,null),$t=ye?.dimensions,Ne=$t&&[$t.length,$t.width,$t.height].every(t=>Number.isFinite(t)&&t>0),Jt=ye?.margin;let tt="box-101",w=500,Tt="",Yt="size",X=!1,x=Ne?$t:null,T=Jt===5||Jt===10?Jt:0,oe,at=null,ee="",ot="all",ae="",Nt="Усі",A="",L="",wt="",S="";const $e=document.querySelector("#app");if(!$e)throw new Error("Root element #app was not found.");function R(){return mt(u.accounts,be)}function K(){const t=te.map(e=>({...e,active:!0,updatedAt:kt}));return mt(u.products,t).filter(e=>e&&typeof e.id=="string"&&typeof e.number=="string"&&Number.isFinite(e.basePrice)&&Number.isFinite(e.dimensions?.length)&&Number.isFinite(e.dimensions?.width)&&Number.isFinite(e.dimensions?.height)).map(e=>({...e,active:e.active!==!1,updatedAt:e.updatedAt||kt}))}function M(){return K().filter(t=>t.active)}function F(t){h(u.products,t)}function B(){return mt(u.orders,ve).map(e=>{if("items"in e&&Array.isArray(e.items))return{...e,statusHistory:Array.isArray(e.statusHistory)&&e.statusHistory.length?e.statusHistory:[{status:e.status,at:e.createdAt}]};const a=e;return{id:a.id,createdAt:a.createdAt,customerName:a.customerName,phone:a.phone,company:a.company,comment:a.comment,items:[{productId:a.productId,productNumber:a.productNumber,dimensions:a.dimensions,quantity:a.quantity,unitPrice:a.unitPrice,total:a.total,priceType:a.priceType}],total:a.total,accountId:a.accountId,status:a.status,statusHistory:[{status:a.status,at:a.createdAt}]}})}function z(){const t=M();return mt(u.cart,[]).filter(e=>t.some(a=>a.id===e.productId)&&e.quantity>0)}function Z(){const t=localStorage.getItem(u.session);return R().find(e=>e.id===t)??null}function Rt(t){const e={...t},a=R().filter(n=>n.id!==t.id&&n.role!==t.role);return h(u.accounts,[e,...a]),localStorage.setItem(u.session,e.id),e}function Ue(t,e,a,n){h(u.accounts,[t,...e]),h(u.orders,a),F(n),localStorage.setItem(u.session,t.id)}async function Ut(){if(!k||!xt())return null;try{const t=await q.me();if(t.role==="admin"){const[e,a,n]=await Promise.all([q.adminClients(),q.adminOrders(),q.adminProducts()]);Ue(t,e,a,n)}else{const[e,a]=await Promise.all([q.myOrders(),q.products()]);Rt(t),h(u.orders,e),F(a)}return t}catch(t){if(t instanceof Dt&&t.status===401)return lt(),localStorage.removeItem(u.session),null;throw t}}function Vt(){const t=M();return t.find(e=>e.id===tt)??t[0]}function Lt(t){return Number.isFinite(t)?Math.min(At,Math.max(1,Math.round(t))):1}function nt(){return mt(u.measurements,[]).filter(t=>t&&typeof t.id=="string"&&[t.dimensions?.length,t.dimensions?.width,t.dimensions?.height].every(e=>Number.isFinite(e)&&Number(e)>0)&&[0,5,10].includes(t.margin))}function st(t){return t===0?"без додаткового запасу":`+${t} мм з кожного боку`}function we(){const t=nt();return t.length?`
    ...
    <div class="saved-measurements__list">
      ${t.map(e=>`
            <button type="button" data-saved-measurement="${d(e.id)}">
              ...
              <span>${st(e.margin)}</span>
            </button>
          `).join("")}
    </div>
  `:""}function Se(){const t=document.querySelector("#saved-measurements");t&&(t.innerHTML=we(),t.hidden=!t.innerHTML)}function _e(t,e){const a=`${t.length}-${t.width}-${t.height}-${e}`,n=nt().filter(i=>`${i.dimensions.length}-${i.dimensions.width}-${i.dimensions.height}-${i.margin}`!==a),s={id:`size-${a}`,dimensions:t,margin:e,createdAt:new Date().toISOString()};h(u.measurements,[s,...n].slice(0,5)),h(u.fit,{dimensions:t,margin:e}),Se()}function He(t,e=!0){x={...t.dimensions},T=t.margin,h(u.fit,{dimensions:x,margin:T});const a=document.querySelector("#fit-form");if(a){a.elements.namedItem("length")?.setAttribute("value",String(x.length)),a.elements.namedItem("width")?.setAttribute("value",String(x.width)),a.elements.namedItem("height")?.setAttribute("value",String(x.height));const s=r=>{const o=a.elements.namedItem(r);o instanceof HTMLInputElement&&(o.value=String(x?.[r]??""))};s("length"),s("width"),s("height");const i=a.querySelector(`input[name="fitMargin"][value="${T}"]`);i&&(i.checked=!0)}const n=document.querySelector("#fit-message");n&&(n.textContent=`Розміри застосовано · ${st(T)}.`,n.className="form-message is-success"),X=!1,gt(),e&&window.setTimeout(()=>document.querySelector("#catalog")?.scrollIntoView({behavior:"smooth"}),180)}function Pt(t){let e=t.replace(/\D/g,"");return e.length===10&&e.startsWith("0")&&(e=`38${e}`),e.length===12&&e.startsWith("380")?`+${e}`:t.trim()}function ut(t){return Pt(t).replace(/\D/g,"")}function d(t){return t.replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#039;")}function O(t){return`${t.length} × ${t.width} × ${t.height} мм`}function Ht(t){const e=t%100,a=t%10;return e>=11&&e<=14?`${t} позицій`:a===1?`${t} позиція`:a>=2&&a<=4?`${t} позиції`:`${t} позицій`}function Qt(t,e){return e?.partner?"Фіксована ціна клієнта":t>=Q?"Оптова ціна":"Роздрібна ціна"}function ft(t,e=!1){const{length:a,width:n,height:s}=t.dimensions,i=170+Math.min(100,a/3),r=58+Math.min(54,s/2.5),o=50+Math.min(44,n/4),c=72,l=e?70:82,g=l-o*.55,f=c+i,v=f+o,b=l+r;return`
  ...
  `}function Xt(){return M().map(t=>`<option value="${d(t.id)}"${t.id===tt?" selected":""}>№${d(t.number)} · ${O(t.dimensions)}</option>`).join("")}function Fe(){const t=Vt();return`
    ...
    <main id="main">
      <section class="hero section">
        ...
        <div class="hero-calculator reveal" aria-label="Швидкий розрахунок">
          ...
          <label class="field">
            <span>Коробка</span>
            <select class="select" id="hero-product-select">${Xt()}</select>
          </label>
          <label class="field">
            <span>Кількість</span>
            <input class="input" id="hero-quantity-input" type="number" min="1" max="${At}" value="${w}" />
          </label>
      ...
      <section class="section fit-section" id="fit">
        ...
        <div class="fit-panel reveal">
          <form class="fit-form" id="fit-form" novalidate>
            ...
            <div class="saved-measurements" id="saved-measurements"${nt().length?"":" hidden"}>${we()}</div>
          </form>
        ...
        <div class="calculator-shell reveal">
          ...
          <div class="calculator-card">
            ...
            <label class="field">
              ...
              <select class="select select--large" id="calculator-product-select">${Xt()}</select>
            </label>
            ...
            <div class="quantity-block">
              ...
              <div class="quantity-control">
                ...
                <input id="quantity-input" type="number" min="1" max="${At}" value="${w}" />
  ...
  `}$e.innerHTML=Fe();const Ct=document.querySelector("#product-grid"),Wt=document.querySelector("#catalog-count");function Gt(t){return new Intl.NumberFormat("uk-UA",{maximumFractionDigits:1}).format(Math.max(0,t))}function je(t){if(!x)return"";const e=Bt(x,t.dimensions,T);if(e.fits){const n=Math.min(...e.clearancesPerSide);return`<div class="product-card__fit"><strong>Підходить</strong><span>мін. ${Gt(n)} мм на бік</span></div>`}const a=Math.max(...e.deficitsPerSide)*2;return`<div class="product-card__fit is-near"><strong>Найближчий розмір</strong><span>бракує до ${Gt(a)} мм</span></div>`}function Be(t){if(!x)return"";const e=Bt(x,t.dimensions,T);if(e.fits)return`<div class="product-modal__fit is-fit"><strong>Коробка підходить</strong><span>${st(T)} враховано у підборі.</span></div>`;const a=Math.max(...e.deficitsPerSide)*2;return`<div class="product-modal__fit is-warning" role="status"><strong>Цей розмір замалий</strong><span>Бракує до ${Gt(a)} мм для обраного запасу. Додайте лише після ручної перевірки.</span></div>`}function ce(t){const e=Z(),a=D(t,1),n=D(t,Q),s=e?.partner?it(t,1,e):null;return`
    <article
  ...
  `}function Re(t){const e=Z(),a=it(t,w,e),n=a*w;return`
    <div class="product-modal">
      ...
      <div class="product-modal__content">
        ...
        <div class="quantity-block quantity-block--modal">
          ...
          <div class="quantity-control">
            <button type="button" data-quantity-step="-100" aria-label="Зменшити кількість на 100">−</button>
            <input id="modal-quantity-input" type="number" min="1" max="${At}" value="${w}" />
            <button type="button" data-quantity-step="100" aria-label="Збільшити кількість на 100">+</button>
          </div>
        ...
        <div class="product-modal__total" aria-live="polite">
          <div><span id="modal-price-tier">${Qt(w,e)}</span><strong id="modal-unit-price">${$(a)} / шт.</strong></div>
  ...
  `}function qe(){const t=document.querySelector("#product-dialog");if(!t?.open||!at)return;const e=M().find(l=>l.id===at);if(!e)return;const a=Z(),n=it(e,w,a),s=t.querySelector("#modal-quantity-input");s&&(s.value=String(w));const i=t.querySelector("#modal-quantity-output");i&&(i.value=`${w.toLocaleString("uk-UA")} шт.`);const r=t.querySelector("#modal-price-tier");r&&(r.textContent=Qt(w,a));const o=t.querySelector("#modal-unit-price");o&&(o.textContent=`${$(n)} / шт.`);const c=t.querySelector("#modal-total");c&&(c.textContent=$(n*w)),t.querySelectorAll("[data-quantity]").forEach(l=>{l.classList.toggle("is-active",Number(l.dataset.quantity)===w)})}function de(t){const e=M().find(s=>s.id===t),a=document.querySelector("#product-dialog"),n=document.querySelector("#product-dialog-content");!e||!a||!n||(at=e.id,ne(e.id),n.innerHTML=Re(e),typeof a.showModal=="function"?a.showModal():a.setAttribute("open",""),qe())}function Ve(){const t=M(),e=Tt.trim().toLocaleLowerCase("uk-UA");return t.filter(n=>{const s=`${n.number} ${n.name} ${O(n.dimensions)}`.toLocaleLowerCase("uk-UA"),i=!e||s.includes(e),r=!x||Bt(x,n.dimensions,T).fits;return i&&r}).sort((n,s)=>Yt==="price"?n.basePrice-s.basePrice:Yt==="number"?n.number.localeCompare(s.number,"uk-UA",{numeric:!0}):Ot(n)-Ot(s))}function Qe(){if(!x)return[];const t=Tt.trim().toLocaleLowerCase("uk-UA");return M().filter(e=>{const a=`${e.number} ${e.name} ${O(e.dimensions)}`.toLocaleLowerCase("uk-UA");return!t||a.includes(t)}).map(e=>{const n=Bt(x,e.dimensions,T).deficitsPerSide.reduce((s,i)=>s+i,0);return{product:e,deficit:n}}).sort((e,a)=>e.deficit-a.deficit||Ot(e.product)-Ot(a.product)).slice(0,3).map(({product:e})=>e)}function U(t=!1){if(!Ct||!Wt)return;const e=document.querySelector("#catalog-more"),a=document.querySelector("#catalog-more-button");if(t){Wt.textContent="Оновлюємо список…",e&&(e.hidden=!0),Ct.innerHTML=Array.from({length:6},()=>'<div class="product-skeleton" aria-hidden="true"><i></i><i></i><i></i></div>').join("");return}const n=Ve(),s=x?` · предмет ${O(x)} · ${st(T)}`:"";if(Wt.textContent=`${n.length} із ${M().length} розмірів${s}`,!n.length){const c=Qe();Ct.innerHTML=`
      <div class="empty-state${c.length?" empty-state--nearest":""}">
        ...
        <p>${T?`Із запасом ${st(T)} точного варіанта немає. Найближчі коробки нижче замалі — це позначено окремо.`:"Змініть габарити предмета або залиште заявку з потрібним розміром."}</p>
        ...
        ${c.length?`<div class="nearest-results"><div class="nearest-results__head"><strong>Найближчі готові розміри</strong><span>Вони не відповідають обраному запасу</span></div><div class="nearest-results__grid">${c.map(ce).join("")}</div></div>`:""}
      </div>
    `,e&&(e.hidden=!0);return}const r=window.matchMedia("(max-width: 680px)").matches&&!Tt.trim()&&!x&&n.length>4,o=r&&!X?n.slice(0,4):n;Ct.innerHTML=o.map(ce).join(""),e&&a&&(e.hidden=!r,a.textContent=X?"Згорнути каталог":`Показати всі ${n.length} розмірів`,a.setAttribute("aria-expanded",String(X)))}function gt(){window.clearTimeout(oe),U(!0),oe=window.setTimeout(()=>U(!1),320)}function J(){const t=Vt(),e=Z(),a=it(t,w,e),n=a*w,s=Qt(w,e);document.querySelectorAll("#calculator-product-select, #hero-product-select").forEach(C=>{C.value=t.id}),document.querySelectorAll("#quantity-input, #hero-quantity-input, #modal-quantity-input").forEach(C=>{C.value=String(w)});const i=document.querySelector("#quantity-output");i&&(i.value=`${w.toLocaleString("uk-UA")} шт.`);const r=document.querySelector("#calculator-preview");r&&(r.classList.remove("is-changing"),r.offsetWidth,r.classList.add("is-changing"),r.innerHTML=ft(t,!0));const o=document.querySelector("#calculator-tier");o&&(o.textContent=s);const c=document.querySelector("#calculator-unit-price");c&&(c.innerHTML=`${$(a)}<small>/ шт.</small>`);const l=document.querySelector("#calculator-total");l&&(l.textContent=$(n));const g=document.querySelector("#hero-price-label");g&&(g.textContent=s);const f=document.querySelector("#hero-total");f&&(f.textContent=$(n));const v=document.querySelector("#hero-unit");v&&(v.textContent=`${$(a)} / шт.`);const b=document.querySelector("#account-price-badge");b&&(b.textContent=e?.partner?"Персональна ціна активна":"Публічна ціна",b.classList.toggle("is-partner",!!e?.partner));const P=document.querySelector("#threshold-note");if(P)if(e?.partner)P.innerHTML=`<strong>Фіксована ціна:</strong> ${$(a)} за одиницю незалежно від тиражу.`;else if(w<Q){const C=Q-w,H=D(t,Q)*Q;P.innerHTML=`Ще <strong>${C.toLocaleString("uk-UA")} шт.</strong> до оптового тарифу. 1000 шт. коштуватимуть ${$(H)}.`}else P.innerHTML=`<strong>Оптовий тариф активний.</strong> Економія проти роздрібної ціни — ${$(w)} на всьому тиражі.`;document.querySelectorAll("[data-quantity]").forEach(C=>{C.classList.toggle("is-active",Number(C.dataset.quantity)===w)}),G(),qe()}function ne(t,e=!1){M().some(a=>a.id===t)&&(tt=t,U(!1),J(),e&&document.querySelector("#calculator")?.scrollIntoView({behavior:"smooth",block:"start"}))}function ct(t){w=Lt(t),J()}function le(t,e){if(!M().some(i=>i.id===t))return;const a=z(),n=a.find(i=>i.productId===t);n?n.quantity=Lt(e):a.push({productId:t,quantity:Lt(e)}),h(u.cart,a),G();const s=document.querySelector("#cart-button");s?.classList.remove("is-updated"),s?.offsetWidth,s?.classList.add("is-updated")}function xe(t,e){const a=z(),n=a.find(s=>s.productId===t);n&&(n.quantity=Lt(e),h(u.cart,a),G())}function ze(t){h(u.cart,z().filter(e=>e.productId!==t)),G()}function G(){const t=document.querySelector("#request-summary"),e=document.querySelector("#cart-count"),a=document.querySelector('#request-form button[type="submit"]'),n=z(),s=Z();if(e&&(e.textContent=String(n.length)),a&&(a.disabled=n.length===0),!t)return;if(!n.length){t.innerHTML=`
    ...
    `;return}let i=0;const r=n.map(o=>{const c=M().find(f=>f.id===o.productId);if(!c)return"";const l=it(c,o.quantity,s),g=l*o.quantity;return i+=g,`
        <article class="cart-item">
          ...
          <label class="cart-item__quantity">
            ...
            <div class="cart-item__quantity-control">
              ...
              <input class="input" type="number" min="1" max="${At}" value="${o.quantity}" data-cart-quantity="${d(c.id)}" />
  ...
  `}function Ke(t){const e=B().find(i=>i.id===t);if(!e)return;const a=new Set(M().map(i=>i.id)),n=e.items.filter(i=>a.has(i.productId)).map(i=>({productId:i.productId,quantity:Lt(i.quantity)}));if(!n.length)return;const s=z().filter(i=>!n.some(r=>r.productId===i.productId));h(u.cart,[...s,...n]),G(),window.location.hash="request",window.setTimeout(()=>document.querySelector("#request")?.scrollIntoView({behavior:"smooth",block:"start"}),80)}function W(){const t=document.querySelector("#account-button"),e=Z();if(!t)return;t.textContent=e?e.name.split(" ")[0]:"Кабінет",t.classList.toggle("is-signed-in",!!e);const a=document.querySelector("#request-account-hint");a&&(a.textContent=e?e.name:"Гість");const n=document.querySelector("#request-form");if(n&&e){const s=(i,r)=>{const o=n.elements.namedItem(i);o instanceof HTMLInputElement&&!o.value&&(o.value=r)};s("name",e.name),s("phone",e.phone),s("company",e.company)}G()}function Je(){const t=Z();if(t){const e=B().filter(o=>o.accountId===t.id).slice().reverse(),a=e.filter(o=>o.status!=="Закрита").length,n=e.reduce((o,c)=>o+c.total,0),s=t.name.split(/\s+/).filter(Boolean).slice(0,2).map(o=>o[0]).join("").toLocaleUpperCase("uk-UA"),i=Vt(),r=it(i,w,t);return`
      <div class="account-dashboard">
        ...
        <div class="account-dashboard__grid">
              ...
              ${e.length?e.map(o=>{const c=o.items.reduce((l,g)=>l+g.quantity,0);return`
                          <article class="account-order">
                            <div class="account-order__main">
                              <span>${d(o.id)}</span>
                              <strong>${Ht(o.items.length)}</strong>
          ...
          <aside class="account-sidebar">
            ...
            ${nt().length?`<article class="account-measurements"><div><p class="technical-label">Збережені розміри</p><span>${nt().length} останніх</span></div><div class="account-measurements__list">${nt().map(o=>`<button type="button" data-saved-measurement="${d(o.id)}"><strong>${O(o.dimensions)}</strong><span>${st(o.margin)}</span></button>`).join("")}</div></article>`:""}
          </aside>
        </div>
      </div>
    `}return`
  ...
  `}function V(){const t=document.querySelector("#account-page-content");t&&(t.innerHTML=Je())}function We(t){return`
  ...
  `}function Ze(){const t=Z(),e=document.querySelector("#profile-dialog"),a=document.querySelector("#profile-dialog-content");!t||!e||!a||(a.innerHTML=We(t),typeof e.showModal=="function"?e.showModal():e.setAttribute("open",""),a.querySelector('input[name="name"]')?.focus())}async function Ye(t){t.classList.add("was-validated");const e=t.querySelector("[data-profile-status]");if(!t.reportValidity()){e&&(e.className="form-status is-error",e.textContent="Перевірте ім’я, телефон і новий пароль.");return}const a=Z();if(!a)return;const n=new FormData(t),s=Pt(String(n.get("phone")??"")),i=R();if(i.some(c=>c.id!==a.id&&ut(c.phone)===ut(s))){e&&(e.className="form-status is-error",e.textContent="Акаунт із таким номером уже існує.");return}const r=String(n.get("password")??"");if(k){try{const c=await q.updateMe({name:String(n.get("name")??"").trim(),phone:s,company:String(n.get("company")??"").trim(),...r?{password:r}:{}});Rt(c),document.querySelector("#profile-dialog")?.close(),W(),V(),J(),U(!1)}catch(c){e&&(e.className="form-status is-error",e.textContent=N(c,"Не вдалося оновити профіль."))}return}const o=i.map(c=>c.id===a.id?{...c,name:String(n.get("name")??"").trim(),phone:s,company:String(n.get("company")??"").trim(),password:r||c.password}:c);h(u.accounts,o),document.querySelector("#profile-dialog")?.close(),W(),V(),J(),U(!1)}function Ft(t,e,a){const n=t.querySelector("[data-auth-status]");n&&(n.textContent=e,n.className=`form-status is-${a}`)}function N(t,e){return t instanceof Dt?t.message:e}function Xe(t,e){const a=ut(t),n=R().find(s=>ut(s.phone)===a&&s.password===e);return n?(localStorage.setItem(u.session,n.id),n):null}async function ue(t,e=!1){if(t.classList.add("was-validated"),!t.reportValidity())return;const a=new FormData(t);let n=null;if(k)try{const s=await q.login(String(a.get("phone")??""),String(a.get("password")??""));e&&s.role!=="admin"?lt():(n=Rt(s),await Ut())}catch(s){Ft(t,N(s,"Сервер авторизації недоступний."),"error");return}else n=Xe(String(a.get("phone")??""),String(a.get("password")??""));if(!n||e&&n.role!=="admin"){Ft(t,e?"Потрібен акаунт менеджера.":"Невірний телефон або пароль.","error");return}W(),J(),U(!1),e?y():(V(),window.location.hash="account")}async function Ge(t){if(t.classList.add("was-validated"),!t.reportValidity())return;const e=new FormData(t),a=Pt(String(e.get("phone")??""));if(k){try{const i=await q.register({name:String(e.get("name")??"").trim(),phone:a,company:String(e.get("company")??"").trim(),password:String(e.get("password")??"")});Rt(i),h(u.orders,[]),W(),J(),U(!1),V(),window.location.hash="account"}catch(i){Ft(t,N(i,"Не вдалося створити акаунт."),"error")}return}const n=R();if(n.some(i=>ut(i.phone)===ut(a))){Ft(t,"Акаунт із таким номером уже існує.","error");return}const s={id:`account-${Date.now().toString(36)}`,name:String(e.get("name")??"").trim(),phone:a,company:String(e.get("company")??"").trim(),password:String(e.get("password")??""),role:"client",partner:!1,fixedMarkup:Zt,createdAt:new Date().toISOString()};n.push(s),h(u.accounts,n),localStorage.setItem(u.session,s.id),W(),J(),U(!1),V(),window.location.hash="account"}async function ta(t){const e=document.querySelector("#request-status"),a=z();if(!a.length){e&&(e.className="form-status is-error",e.textContent="Додайте хоча б одну коробку до кошика.");return}if(t.classList.add("was-validated"),!t.reportValidity()){e&&(e.className="form-status is-error",e.textContent="Перевірте обов’язкові поля та згоду.");return}const n=new FormData(t),s=Z(),i=Pt(String(n.get("phone")??"")),r=s??R().find(v=>Pt(v.phone)===i),o=a.flatMap(v=>{const b=M().find(C=>C.id===v.productId);if(!b)return[];const P=it(b,v.quantity,s);return[{productId:b.id,productNumber:b.number,dimensions:b.dimensions,quantity:v.quantity,unitPrice:P,total:P*v.quantity,priceType:Qt(v.quantity,s)}]}),c=o.reduce((v,b)=>v+b.total,0);if(k){const v=t.querySelector('button[type="submit"]');v&&(v.disabled=!0,v.dataset.originalText=v.textContent??"",v.textContent="Зберігаємо заявку…");try{const b=await q.createOrder({customerName:String(n.get("name")??"").trim(),phone:i,company:String(n.get("company")??"").trim(),comment:String(n.get("comment")??"").trim(),items:a.map(P=>({productId:P.productId,quantity:P.quantity}))});h(u.orders,[...B().filter(P=>P.id!==b.id),b]),h(u.cart,[]),G(),V(),e&&(e.className="form-status is-success",e.innerHTML=`<strong>Заявку ${d(b.id)} створено.</strong><span>${Ht(b.items.length)} на суму ${$(b.total)}. Менеджер побачить її в адмінці.</span>`)}catch(b){e&&(e.className="form-status is-error",e.textContent=N(b,"Не вдалося передати заявку на сервер."))}finally{v&&(v.disabled=!1,v.textContent=v.dataset.originalText||"Надіслати заявку",v.focus())}return}const l=new Date().toISOString(),g={id:`TP-${Date.now().toString(36).toUpperCase()}`,createdAt:l,customerName:String(n.get("name")??"").trim(),phone:i,company:String(n.get("company")??"").trim(),comment:String(n.get("comment")??"").trim(),items:o,total:c,accountId:r?.id,status:"Нова",statusHistory:[{status:"Нова",at:l}]},f=B();f.push(g),h(u.orders,f),h(u.cart,[]),G(),V(),e&&(e.className="form-status is-success",e.innerHTML=`<strong>Заявку ${d(g.id)} створено.</strong><span>${Ht(g.items.length)} на суму ${$(g.total)}. Номер можна повідомити менеджеру.</span>`),t.querySelector('button[type="submit"]')?.focus()}const ea=["Нова","У роботі","Уточнення","Підтверджена","Закрита"];function pe(t){return t==="Нова"?"is-new":t==="У роботі"?"is-progress":t==="Уточнення"?"is-clarifying":t==="Підтверджена"?"is-confirmed":"is-closed"}function aa(t){return`
    <div class="order-status-control ${pe(t.status)}" data-order-status-control>
      ...
      <div class="order-status-control__menu" role="listbox" aria-label="Статус заявки ${d(t.id)}" hidden>
        ${ea.map(e=>`
            <button class="${pe(e)}" type="button" role="option" aria-selected="${e===t.status}" data-order-status-option="${d(e)}" data-order-id="${d(t.id)}">
              <span class="order-status-control__dot" aria-hidden="true"></span>
              <span>${d(e)}</span>
  ...
  `}function St(t){const e=t.getFullYear(),a=String(t.getMonth()+1).padStart(2,"0"),n=String(t.getDate()).padStart(2,"0");return`${e}-${a}-${n}`}function _t(t){const[e,a,n]=t.split("-").map(Number);return new Date(e,Math.max(0,(a||1)-1),n||1,12)}function rt(t){return _t(t).toLocaleDateString("uk-UA",{day:"2-digit",month:"2-digit",year:"numeric"})}function Ae(t=!1){const a=St(new Date),s=_t(wt||A||a),i=new Date(s.getFullYear(),s.getMonth(),1,12);wt=St(i);const r=(i.getDay()+6)%7,o=new Date(i);o.setDate(i.getDate()-r);const c=Array.from({length:42},(P,C)=>{const H=new Date(o);H.setDate(o.getDate()+C);const E=St(H),Mt=H.getMonth()!==i.getMonth(),ht=E===A,bt=E===L,vt=ht||bt,yt=!!(A&&L&&E>A&&E<L);return`<button class="${[Mt?"is-outside":"",yt?"is-in-range":"",ht?"is-range-start":"",bt?"is-range-end":"",vt?"is-selected":"",E===a?"is-today":""].filter(Boolean).join(" ")}" type="button" data-calendar-date="${E}" aria-label="${H.toLocaleDateString("uk-UA",{day:"numeric",month:"long",year:"numeric"})}" aria-pressed="${vt}">${H.getDate()}</button>`}).join(""),l=A?L&&L!==A?`${rt(A)} — ${rt(L)}`:rt(A):"Усі дати",g=A&&L?Math.round((_t(L).getTime()-_t(A).getTime())/864e5)+1:0,f=A?L?g===1?"Обрано один день":`Обрано ${g} дн.`:"Тепер оберіть кінець":"Оберіть початок",v=A?L?`${rt(A)} — ${rt(L)}`:`Початок: ${rt(A)}`:"Перший клік — початкова дата",b=i.toLocaleDateString("uk-UA",{month:"long",year:"numeric"});return`
  ...
  `}function na(){return window.location.hash==="#admin-orders"?"orders":window.location.hash==="#admin-clients"?"clients":window.location.hash==="#admin-products"?"products":"overview"}function sa(t,e,a,n){return[{view:"overview",href:"#admin",label:"Огляд"},{view:"orders",href:"#admin-orders",label:"Замовлення",count:e},{view:"clients",href:"#admin-clients",label:"Клієнти",count:a},{view:"products",href:"#admin-products",label:"Товари",count:n}].map((i,r)=>`
      ...
      `).join("")}function ia(t,e,a){const n=B(),s=R().filter(o=>o.role==="client"),i=K().length,r=S?`<div class="admin-notice" role="status"><span>Готово</span><p>${d(S)}</p></div>`:"";return S="",`
  ...
  `}function ke(t){const e=`${t.id} ${t.customerName} ${t.phone} ${t.company}`.toLocaleLowerCase("uk-UA");return`
  ...
  `}function ra(t,e){const a=t.filter(r=>r.status!=="Закрита").length,n=t.reduce((r,o)=>r+o.total,0),s=M().length,i=t.slice(0,3);return`
    ...
    <section class="admin-section">
      ...
      <div class="orders-list">
        ${i.length?i.map(ke).join(""):'<div class="admin-empty"><h3>Заявок ще немає.</h3><p>Нові замовлення з сайту з’являться тут.</p></div>'}
      </div>
    </section>
  `}function oa(t){const e=["Усі","Нова","У роботі","Уточнення","Підтверджена","Закрита"];return`
    ...
    <div class="admin-toolbar">
      <label class="admin-search"><span class="sr-only">Пошук заявок</span><input id="admin-order-search" type="search" value="${d(ae)}" placeholder="Номер, ім’я або телефон" /></label>
      ${Ae()}
      <div class="admin-filter-chips" aria-label="Фільтр за статусом">
        ${e.map(a=>`<button class="${Nt===a?"is-active":""}" type="button" data-admin-order-filter="${a}">${a}</button>`).join("")}
      </div>
    </div>
    ...
    <div class="orders-list" id="admin-orders-list">
      ${t.length?t.map(ke).join(""):'<div class="admin-empty"><h3>Заявок ще немає.</h3><p>Нові замовлення з сайту з’являться тут.</p></div>'}
    </div>
  ...
  `}function se(){const t=ee.trim().toLocaleLowerCase("uk-UA");return K().filter(e=>{const a=!t||`${e.number} ${e.name} ${O(e.dimensions)}`.toLocaleLowerCase("uk-UA").includes(t),n=ot==="all"||(ot==="active"?e.active:!e.active);return a&&n})}function Le(){const t=se();return t.length?t.map(e=>`
      ...
      `).join(""):'<div class="admin-empty"><h3>Нічого не знайдено.</h3><p>Змініть пошук або фільтр видимості.</p></div>'}function da(){return`
    ...
    <div class="admin-toolbar admin-toolbar--products">
      <label class="admin-search"><span class="sr-only">Пошук товарів</span><input id="admin-product-search" type="search" value="${d(ee)}" placeholder="Номер або розмір" /></label>
    ...
    <div class="admin-results-meta"><strong id="admin-product-count">${se().length}</strong><span>товарів показано</span></div>
    <div class="admin-products-grid" id="admin-product-list">${Le()}</div>
  `}function y(){const t=document.querySelector("#admin-content");if(!t)return;const e=Z();if(!e||e.role!=="admin"){t.innerHTML=`
    ...
    `;return}const a=B().slice().reverse(),n=R().filter(r=>r.role==="client"),s=na();let i=ra(a,n);s==="orders"&&(i=oa(a)),s==="clients"&&(i=ca(n)),s==="products"&&(i=da()),t.innerHTML=ia(e,s,i),s==="orders"&&qt()}function qt(){const t=ae.trim().toLocaleLowerCase("uk-UA");let e=0;document.querySelectorAll("[data-admin-order]").forEach(n=>{const s=!t||(n.dataset.search??"").includes(t),i=Nt==="Усі"||n.dataset.status===Nt,r=n.dataset.date??"",o=!A||!L&&r===A||!!(L&&r>=A&&r<=L);n.hidden=!(s&&i&&o),n.hidden||(e+=1)});const a=document.querySelector("#admin-order-count");a&&(a.textContent=String(e))}function Et(t){document.querySelectorAll("[data-order-status-control]").forEach(e=>{e!==t&&(e.classList.remove("is-open"),e.querySelector(".order-status-control__menu")?.setAttribute("hidden",""),e.querySelector("[data-order-status-trigger]")?.setAttribute("aria-expanded","false"))})}function Pe(){const t=document.querySelector("[data-admin-calendar]");t&&(t.classList.remove("is-open"),t.querySelector(".admin-calendar__popover")?.setAttribute("hidden",""),t.querySelector("[data-calendar-trigger]")?.setAttribute("aria-expanded","false"))}function It(t,e){const a=document.querySelector("[data-admin-calendar]");a&&(a.outerHTML=Ae(t),e&&window.requestAnimationFrame(()=>document.querySelector(`[data-admin-calendar] ${e}`)?.focus()))}async function la(t,e){const a=B(),n=a.find(i=>i.id===t);if(!n||n.status===e)return;if(k){try{const i=await q.updateOrder(t,{status:e});h(u.orders,a.map(r=>r.id===t?i:r))}catch(i){S=N(i,"Не вдалося змінити статус заявки.")}y();return}const s=n.status;n.status=e,n.statusHistory=[...n.statusHistory??[{status:s,at:n.createdAt}],{status:e,at:new Date().toISOString()}],h(u.orders,a),y()}function ua(t){const e=t.trim().toLocaleLowerCase("uk-UA");document.querySelectorAll("[data-admin-client]").forEach(a=>{a.hidden=!!e&&!(a.dataset.search??"").includes(e)})}function pa(){const t=document.querySelector("#admin-product-list");t&&(t.innerHTML=Le());const e=document.querySelector("#admin-product-count");e&&(e.textContent=String(se().length))}function j(){const t=M();if(!t.length)return;t.some(s=>s.id===tt)||(tt=t[0].id);const e=Xt();document.querySelectorAll("#calculator-product-select, #hero-product-select").forEach(s=>{s.innerHTML=e,s.value=tt});const a=document.querySelector("#hero-product-count");a&&(a.textContent=String(t.length));const n=document.querySelector("#catalog-ready-label");n&&(n.innerHTML=`<span></span> ${t.length} готових розмірів`),U(!1),J(),G()}function ma(t){const e=!!t,a=t??{id:"",number:"",name:"",dimensions:{length:180,width:120,height:50},basePrice:5,active:!0};return`
  ...
  `}function me(t){const e=document.querySelector("#admin-product-dialog"),a=document.querySelector("#admin-product-editor");if(!e||!a)return;const n=t?K().find(s=>s.id===t):void 0;a.innerHTML=ma(n),typeof e.showModal=="function"?e.showModal():e.setAttribute("open",""),a.querySelector('input[name="number"]')?.focus()}async function fa(t){t.classList.add("was-validated");const e=t.querySelector("[data-product-form-status]");if(!t.reportValidity()){e&&(e.className="form-status is-error",e.textContent="Перевірте обов’язкові поля.");return}const a=new FormData(t),n=String(a.get("productId")??""),s=String(a.get("number")??"").trim(),i=K(),r=i.find(f=>f.id===n);if(!he.test(s)){e&&(e.className="form-status is-error",e.textContent="У номері можна використовувати літери, цифри, крапку, дефіс і підкреслення.");return}if(i.some(f=>f.number.toLocaleLowerCase("uk-UA")===s.toLocaleLowerCase("uk-UA")&&f.id!==n)){e&&(e.className="form-status is-error",e.textContent="Товар із таким номером уже існує.");return}const o=a.get("active")==="on";if(r?.active&&!o&&M().length<=1){e&&(e.className="form-status is-error",e.textContent="У каталозі має залишитися хоча б один активний товар.");return}const c=r?.id??`box-${s.toLocaleLowerCase("uk-UA").replace(/[^a-zа-яіїєґ0-9]+/giu,"-")}-${Date.now().toString(36)}`,l={...r,id:c,number:s,name:String(a.get("name")??"").trim()||`Самозбірна коробка №${s}`,dimensions:{length:Number(a.get("length")),width:Number(a.get("width")),height:Number(a.get("height"))},basePrice:Number(a.get("basePrice")),active:o,updatedAt:new Date().toISOString()};if(k){try{const f=r?await q.updateProduct(r.id,l):await q.createProduct({number:l.number,name:l.name,dimensions:l.dimensions,basePrice:l.basePrice,sourceQuantity:l.sourceQuantity,active:l.active}),v=r?i.map(b=>b.id===r.id?f:b):[...i,f];F(v),j(),document.querySelector("#admin-product-dialog")?.close(),S=r?`Товар №${s} оновлено.`:`Товар №${s} додано до каталогу.`,y()}catch(f){e&&(e.className="form-status is-error",e.textContent=N(f,"Не вдалося зберегти товар на сервері."))}return}const g=r?i.map(f=>f.id===r.id?l:f):[...i,l];F(g),j(),document.querySelector("#admin-product-dialog")?.close(),S=r?`Товар №${s} оновлено.`:`Товар №${s} додано до каталогу.`,y()}function fe(t,e){const a=new Blob([JSON.stringify(e,null,2)],{type:"application/json"}),n=URL.createObjectURL(a),s=document.createElement("a");s.href=n,s.download=t,s.click(),window.setTimeout(()=>URL.revokeObjectURL(n),0)}function ga(t,e,a){const n=new Blob([e],{type:a}),s=URL.createObjectURL(n),i=document.createElement("a");i.href=s,i.download=t,i.click(),window.setTimeout(()=>URL.revokeObjectURL(s),0)}function ha(t){const e=String(t);return/[;"\n\r]/.test(e)?`"${e.replaceAll('"','""')}"`:e}function ba(){const t=["number","name","length","width","height","basePrice","active"],e=K().map(a=>[a.number,a.name,a.dimensions.length,a.dimensions.width,a.dimensions.height,a.basePrice,a.active].map(ha).join(";"));return`\uFEFF${[t.join(";"),...e].join(`\r
`)}`}function va(){return{version:1,createdAt:new Date().toISOString(),accounts:R(),orders:B(),products:K(),cart:z(),measurements:nt(),fit:x?{dimensions:x,margin:T}:null}}function ya(t){if(!t||typeof t!="object")return!1;const e=t;if(e.version!==1||!Array.isArray(e.accounts)||!Array.isArray(e.orders)||!Array.isArray(e.products))return!1;const a=e.accounts.every(i=>i&&typeof i.id=="string"&&typeof i.phone=="string"&&(i.role==="admin"||i.role==="client")),n=e.products.every(i=>i&&typeof i.id=="string"&&typeof i.number=="string"&&Number.isFinite(i.basePrice)&&[i.dimensions?.length,i.dimensions?.width,i.dimensions?.height].every(r=>Number.isFinite(r)&&Number(r)>0)),s=e.orders.every(i=>i&&typeof i.id=="string"&&typeof i.phone=="string"&&Array.isArray(i.items)&&Number.isFinite(i.total));return a&&n&&s&&e.accounts.some(i=>i.role==="admin")}async function $a(t){const e=t.files?.[0];if(e){if(k){S="Серверну копію можна завантажити, а відновлення виконується тільки на сервері адміністратором інфраструктури.",t.value="",y();return}try{const a=JSON.parse(await e.text());if(!ya(a))throw new Error("Файл не є коректною резервною копією ToffiPacks.");if(!window.confirm("Відновити локальні дані з цієї копії? Поточні заявки, клієнти й товари буде замінено."))return;h(u.accounts,a.accounts),h(u.orders,a.orders),h(u.products,a.products),h(u.cart,Array.isArray(a.cart)?a.cart:[]),h(u.measurements,Array.isArray(a.measurements)?a.measurements:[]),a.fit?h(u.fit,a.fit):localStorage.removeItem(u.fit),x=a.fit?.dimensions??null,T=a.fit?.margin===5||a.fit?.margin===10?a.fit.margin:0,R().some(n=>n.id===localStorage.getItem(u.session))||localStorage.removeItem(u.session),j(),W(),V(),S=`Резервну копію від ${new Date(a.createdAt).toLocaleString("uk-UA")} відновлено.`,y()}catch(a){S=a instanceof Error?a.message:"Не вдалося відновити резервну копію.",y()}finally{t.value=""}}}function wa(t){const e=[];let a=[],n="",s=!1;for(let i=0;i<t.length;i+=1){const r=t[i];r==='"'?s&&t[i+1]==='"'?(n+='"',i+=1):s=!s:r===";"&&!s?(a.push(n.trim()),n=""):(r===`
`||r==="\r")&&!s?(r==="\r"&&t[i+1]===`
`&&(i+=1),a.push(n.trim()),a.some(Boolean)&&e.push(a),a=[],n=""):n+=r}return a.push(n.trim()),a.some(Boolean)&&e.push(a),e}function Sa(t){const e=wa(t.replace(/^\uFEFF/,"")),a=e.shift()?.map(l=>l.trim())??[],n=["number","name","length","width","height","basePrice","active"];if(!n.every(l=>a.includes(l)))throw new Error(`Потрібні колонки: ${n.join(", ")}`);const s=Object.fromEntries(a.map((l,g)=>[l,g])),i=K(),r=new Map(i.map(l=>[l.number.toLocaleLowerCase("uk-UA"),l])),o=e.map(l=>{const g=E=>l[s[E]]?.trim()??"",f=g("number"),v=E=>Number(g(E).replace(",",".")),b={length:v("length"),width:v("width"),height:v("height")},P=v("basePrice");if(!he.test(f)||!Object.values(b).every(E=>Number.isFinite(E)&&E>0)||!Number.isFinite(P)||P<=0)throw new Error(`Некоректні дані для коробки ${f||"без номера"}.`);const C=r.get(f.toLocaleLowerCase("uk-UA")),H=g("active").toLocaleLowerCase("uk-UA");return{...C,id:C?.id??`box-${f.toLocaleLowerCase("uk-UA").replace(/[^a-zа-яіїєґ0-9]+/giu,"-")}-${Date.now().toString(36)}`,number:f,name:g("name")||C?.name||`Самозбірна коробка №${f}`,dimensions:b,basePrice:P,active:!["false","0","ні","no"].includes(H),updatedAt:new Date().toISOString()}}),c=new Set(o.map(l=>l.number.toLocaleLowerCase("uk-UA")));return[...i.filter(l=>!c.has(l.number.toLocaleLowerCase("uk-UA"))),...o]}async function _a(t){const e=t.files?.[0];if(e)try{const a=Sa(await e.text());if(!window.confirm(`Імпортувати ${a.length} товарів? Позиції з однаковими номерами буде оновлено.`))return;if(k){const n=await q.adminProducts();for(const s of a){const i=n.find(r=>r.id===s.id||r.number.toLocaleLowerCase("uk-UA")===s.number.toLocaleLowerCase("uk-UA"));i?await q.updateProduct(i.id,s):await q.createProduct({number:s.number,name:s.name,dimensions:s.dimensions,basePrice:s.basePrice,sourceQuantity:s.sourceQuantity,active:s.active})}F(await q.adminProducts()),j(),S="CSV імпортовано на сервер. Каталог оновлено.",y();return}F(a),j(),S="CSV імпортовано. Каталог оновлено.",y()}catch(a){S=a instanceof Error?a.message:"Не вдалося прочитати CSV.",y()}finally{t.value=""}}function zt(){const t=document.querySelector("#admin-page"),e=document.querySelector("#account-page"),a=document.querySelector("#main"),n=document.querySelector(".site-header"),s=document.querySelector(".site-footer"),i=document.querySelector(".demo-strip"),r=["#admin","#admin-orders","#admin-clients","#admin-products"].includes(window.location.hash),o=window.location.hash==="#account";t&&(t.hidden=!r),e&&(e.hidden=!o),a&&(a.hidden=r||o),n&&(n.hidden=r||o),s&&(s.hidden=r||o),i&&(i.hidden=r||o),document.body.classList.toggle("is-admin",r),document.body.classList.toggle("is-account",o),r?(y(),k&&xt()&&Ut().then(()=>y()).catch(c=>{S=N(c,"Не вдалося оновити дані адмінки."),y()}),window.scrollTo({top:0})):o&&(V(),k&&xt()&&Ut().then(()=>{W(),V()}).catch(()=>{}),window.scrollTo({top:0}))}function qa(){const t=document.querySelectorAll(".reveal");if(!("IntersectionObserver"in window)){t.forEach(a=>a.classList.add("is-visible"));return}const e=new IntersectionObserver(a=>{a.forEach(n=>{n.isIntersecting&&(n.target.classList.add("is-visible"),e.unobserve(n.target))})},{threshold:.12});t.forEach(a=>e.observe(a))}document.querySelector("#menu-button")?.addEventListener("click",t=>{const e=t.currentTarget,a=document.querySelector("#site-nav"),n=e.getAttribute("aria-expanded")!=="true";e.setAttribute("aria-expanded",String(n)),a?.classList.toggle("is-open",n)});document.querySelectorAll(".site-nav a").forEach(t=>{t.addEventListener("click",()=>{document.querySelector("#site-nav")?.classList.remove("is-open"),document.querySelector("#menu-button")?.setAttribute("aria-expanded","false")})});document.querySelector('.site-header .brand[href="#top"]')?.addEventListener("click",t=>{t.preventDefault(),window.location.hash!=="#top"&&(window.history.pushState(null,"","#top"),zt()),window.scrollTo({top:0,behavior:window.matchMedia("(prefers-reduced-motion: reduce)").matches?"auto":"smooth"})});document.querySelector("#fit-form")?.addEventListener("submit",t=>{t.preventDefault();const e=t.currentTarget;e.classList.add("was-validated");const a=document.querySelector("#fit-message");if(!e.reportValidity()){a&&(a.textContent="Вкажіть три додатні розміри.",a.className="form-message is-error");return}const n=new FormData(e);x={length:Number(n.get("length")),width:Number(n.get("width")),height:Number(n.get("height"))};const s=Number(n.get("fitMargin"));T=s===5||s===10?s:0,_e(x,T),X=!1,a&&(a.textContent=`Розміри застосовано · ${st(T)}.`,a.className="form-message is-success"),gt(),window.setTimeout(()=>document.querySelector("#catalog")?.scrollIntoView({behavior:"smooth"}),180)});document.querySelector("#catalog-search")?.addEventListener("input",t=>{Tt=t.currentTarget.value,X=!1,gt()});const et=document.querySelector("#catalog-sort"),dt=et?.querySelector(".catalog-sort__trigger"),pt=et?.querySelector(".catalog-sort__menu"),Y=Array.from(et?.querySelectorAll("[data-sort-value]")??[]);function Kt(t=!1){!dt||!pt||(dt.setAttribute("aria-expanded","false"),pt.hidden=!0,et?.classList.remove("is-open"),t&&dt.focus())}function Te(){!dt||!pt||(dt.setAttribute("aria-expanded","true"),pt.hidden=!1,et?.classList.add("is-open"))}function xa(t){const e=Y.find(n=>n.dataset.sortValue===t),a=document.querySelector("#catalog-sort-value");!e||!et||!a||(Yt=t,et.dataset.value=t,a.textContent=e.querySelector("span")?.textContent??e.textContent,Y.forEach(n=>{n.setAttribute("aria-selected",String(n===e))}),Kt(!0),gt())}dt?.addEventListener("click",()=>{pt?.hidden?Te():Kt()});Y.forEach(t=>{t.addEventListener("click",()=>{xa(t.dataset.sortValue)})});et?.addEventListener("keydown",t=>{const e=Y.indexOf(document.activeElement),a=Y.findIndex(s=>s.getAttribute("aria-selected")==="true");if(t.key==="Escape"){t.preventDefault(),Kt(!0);return}if(t.key!=="ArrowDown"&&t.key!=="ArrowUp"&&t.key!=="Home"&&t.key!=="End")return;t.preventDefault(),pt?.hidden&&Te();let n=e>=0?e:a;t.key==="Home"&&(n=0),t.key==="End"&&(n=Y.length-1),t.key==="ArrowDown"&&(n=(n+1)%Y.length),t.key==="ArrowUp"&&(n=(n-1+Y.length)%Y.length),Y[n]?.focus()});document.addEventListener("click",t=>{et?.contains(t.target)||Kt()});document.querySelector("#reset-catalog")?.addEventListener("click",()=>{x=null,T=0,Tt="",X=!1;const t=document.querySelector("#catalog-search");t&&(t.value="");const e=document.querySelector("#fit-message");e&&(e.textContent=""),localStorage.removeItem(u.fit),gt()});document.querySelector("#catalog-more-button")?.addEventListener("click",()=>{X=!X,U(!1),X||document.querySelector("#catalog")?.scrollIntoView({behavior:"smooth",block:"start"})});window.addEventListener("resize",()=>U(!1));document.querySelector("#calculator-product-select")?.addEventListener("change",t=>{ne(t.currentTarget.value)});document.querySelector("#hero-product-select")?.addEventListener("change",t=>{ne(t.currentTarget.value)});document.querySelector("#quantity-input")?.addEventListener("input",t=>{ct(Number(t.currentTarget.value))});document.querySelector("#hero-quantity-input")?.addEventListener("input",t=>{ct(Number(t.currentTarget.value))});document.querySelector("#request-form")?.addEventListener("submit",t=>{t.preventDefault(),ta(t.currentTarget)});document.addEventListener("click",t=>{const e=t.target;e.closest("[data-order-status-control]")||Et(),e.closest("[data-admin-calendar]")||Pe();const a=e.closest("[data-order-status-trigger]");if(a){const p=a.closest("[data-order-status-control]"),m=p?.querySelector(".order-status-control__menu");if(!p||!m)return;const _=m.hidden;Et(p),m.hidden=!_,p.classList.toggle("is-open",_),a.setAttribute("aria-expanded",String(_));return}const n=e.closest("[data-order-status-option]");if(n?.dataset.orderId&&n.dataset.orderStatusOption){la(n.dataset.orderId,n.dataset.orderStatusOption);return}const s=e.closest("[data-calendar-trigger]");if(s){const p=s.closest("[data-admin-calendar]"),m=p?.querySelector(".admin-calendar__popover");if(!p||!m)return;const _=m.hidden;Et(),m.hidden=!_,p.classList.toggle("is-open",_),s.setAttribute("aria-expanded",String(_));return}const i=e.closest("[data-calendar-month]");if(i?.dataset.calendarMonth){const p=_t(wt||St(new Date));p.setMonth(p.getMonth()+Number(i.dataset.calendarMonth),1),wt=St(p),It(!0,`[data-calendar-month="${i.dataset.calendarMonth}"]`);return}const r=e.closest("[data-calendar-date]");if(r?.dataset.calendarDate){const p=r.dataset.calendarDate;wt=p,!A||L?(A=p,L="",It(!0,`[data-calendar-date="${p}"]`)):(L=p,L<A&&([A,L]=[L,A]),It(!1)),qt();return}if(e.closest("[data-calendar-clear]")){A="",L="",It(!1),qt();return}const o=e.closest("[data-saved-measurement]");if(o?.dataset.savedMeasurement){const p=nt().find(m=>m.id===o.dataset.savedMeasurement);p&&He(p);return}if(e.closest("[data-clear-measurements]")){localStorage.removeItem(u.measurements),Se(),V();return}if(e.closest("[data-use-tight-fit]")&&x){T=0,_e(x,T);const p=document.querySelector('#fit-form input[name="fitMargin"][value="0"]');p&&(p.checked=!0),gt();return}const c=e.closest("[data-open-product]");if(c?.dataset.openProduct){de(c.dataset.openProduct);return}const l=e.closest("[data-quantity]");if(l?.dataset.quantity){ct(Number(l.dataset.quantity));return}const g=e.closest("[data-quantity-step]");if(g?.dataset.quantityStep){ct(w+Number(g.dataset.quantityStep));return}if(e.closest("[data-product-to-cart]")){le(at??tt,w),document.querySelector("#product-dialog")?.close(),at=null;return}if(e.closest("[data-add-selected-to-cart]")){le(tt,w);return}const f=e.closest("[data-cart-step]");if(f?.dataset.cartProduct&&f.dataset.cartStep){const p=z().find(m=>m.productId===f.dataset.cartProduct);p&&xe(p.productId,p.quantity+Number(f.dataset.cartStep));return}const v=e.closest("[data-edit-cart]");if(v?.dataset.editCart){const p=z().find(m=>m.productId===v.dataset.editCart);p&&(ct(p.quantity),de(p.productId));return}if(e.closest("[data-clear-cart]")){window.confirm("Очистити всі позиції кошика?")&&(h(u.cart,[]),G());return}const b=e.closest("[data-repeat-order]");if(b?.dataset.repeatOrder){Ke(b.dataset.repeatOrder);return}const P=e.closest("[data-remove-cart]");if(P?.dataset.removeCart){ze(P.dataset.removeCart);return}if(e.closest("[data-product-to-calculator]")){document.querySelector("#product-dialog")?.close(),at=null,window.location.hash="calculator",document.querySelector("#calculator")?.scrollIntoView({behavior:"smooth",block:"start"});return}const C=e.closest("[data-close-dialog]");if(C){C.closest("dialog")?.close(),at=null;return}if(e.closest("[data-edit-profile]")){Ze();return}if(e.closest("[data-close-profile]")){document.querySelector("#profile-dialog")?.close();return}const H=e.closest("[data-auth-tab]");if(H?.dataset.authTab){const p=H.closest(".auth-forms");p?.querySelectorAll("[data-auth-tab]").forEach(m=>{const _=m.dataset.authTab===H.dataset.authTab;m.classList.toggle("is-active",_),m.setAttribute("aria-selected",String(_))}),p?.querySelectorAll("[data-auth-panel]").forEach(m=>{m.hidden=m.dataset.authPanel!==H.dataset.authTab});return}if(e.closest("#logout-button")){k&&q.logout().catch(()=>lt()),localStorage.removeItem(u.session),W(),J(),U(!1),V();return}if(e.closest("[data-create-product]")){me();return}const E=e.closest("[data-edit-product]");if(E?.dataset.editProduct){me(E.dataset.editProduct);return}if(e.closest("[data-close-admin-product]")){document.querySelector("#admin-product-dialog")?.close();return}const Mt=e.closest("[data-toggle-product]");if(Mt?.dataset.toggleProduct){const p=K(),m=p.find(_=>_.id===Mt.dataset.toggleProduct);if(m){if(k){q.updateProduct(m.id,{active:!m.active}).then(_=>{F(p.map(ie=>ie.id===m.id?_:ie)),j(),S=_.active?`Товар №${_.number} повернуто на сайт.`:`Товар №${_.number} приховано.`,y()}).catch(_=>{S=N(_,"Не вдалося змінити видимість товару."),y()});return}m.active&&M().length<=1?S="У каталозі має залишитися хоча б один активний товар.":(m.active=!m.active,m.updatedAt=new Date().toISOString(),F(p),j(),S=m.active?`Товар №${m.number} повернуто на сайт.`:`Товар №${m.number} приховано.`),y()}return}const ht=e.closest("[data-delete-product]");if(ht?.dataset.deleteProduct){const p=K(),m=p.find(_=>_.id===ht.dataset.deleteProduct);if(!m)return;if(m.active&&M().length<=1){S="Не можна видалити останній активний товар.",y();return}if(window.confirm(`Видалити коробку №${m.number}? Цю дію не можна скасувати.`)){if(k){q.deleteProduct(m.id).then(()=>{F(p.filter(_=>_.id!==m.id)),h(u.cart,z().filter(_=>_.productId!==m.id)),j(),S=`Товар №${m.number} видалено.`,y()}).catch(_=>{S=N(_,"Не вдалося видалити товар."),y()});return}F(p.filter(_=>_.id!==m.id)),h(u.cart,z().filter(_=>_.productId!==m.id)),j(),S=`Товар №${m.number} видалено.`,y()}return}const bt=e.closest("[data-delete-order]");if(bt?.dataset.deleteOrder){const p=B().find(m=>m.id===bt.dataset.deleteOrder);if(!p)return;if(window.confirm(`Видалити заявку ${p.id} від ${p.customerName}? Цю дію не можна скасувати.`)){if(k){q.deleteOrder(p.id).then(()=>{h(u.orders,B().filter(m=>m.id!==p.id)),S=`Заявку ${p.id} видалено.`,y()}).catch(m=>{S=N(m,"Не вдалося видалити заявку."),y()});return}h(u.orders,B().filter(m=>m.id!==p.id)),S=`Заявку ${p.id} видалено.`,y()}return}const vt=e.closest("[data-product-filter]");if(vt?.dataset.productFilter){ot=vt.dataset.productFilter,y();return}const yt=e.closest("[data-admin-order-filter]");if(yt?.dataset.adminOrderFilter){Nt=yt.dataset.adminOrderFilter,document.querySelectorAll("[data-admin-order-filter]").forEach(p=>{p.classList.toggle("is-active",p===yt)}),qt();return}if(e.closest("[data-export-backup]")){if(k){q.backup().then(p=>{fe(`toffipacks-server-backup-${new Date().toISOString().slice(0,10)}.json`,p)}).catch(p=>{S=N(p,"Не вдалося завантажити серверну копію."),y()});return}fe(`toffipacks-backup-${new Date().toISOString().slice(0,10)}.json`,va());return}if(e.closest("[data-export-products]")){ga(`toffipacks-products-${new Date().toISOString().slice(0,10)}.csv`,ba(),"text/csv;charset=utf-8");return}if(e.closest("[data-reset-products]")){if(window.confirm("Відновити початковий каталог? Усі ручні зміни товарів буде втрачено.")){if(k){q.resetProducts().then(p=>{F(p),j(),S="Початковий каталог відновлено.",y()}).catch(p=>{S=N(p,"Не вдалося відновити каталог."),y()});return}F(te.map(p=>({...p,active:!0,updatedAt:new Date().toISOString()}))),j(),S="Початковий каталог відновлено.",y()}return}if(e.closest("#admin-logout")){k&&q.logout().catch(()=>lt()),localStorage.removeItem(u.session),W(),J(),U(!1),window.location.hash="admin",y();return}});document.addEventListener("keydown",t=>{const e=t.target,a=e.closest("[data-order-status-trigger]");if(a&&(t.key==="ArrowDown"||t.key==="ArrowUp")){t.preventDefault();const o=a.closest("[data-order-status-control]");o?.querySelector(".order-status-control__menu")?.hidden&&a.click();const l=Array.from(o?.querySelectorAll("[data-order-status-option]")??[]),g=Math.max(0,l.findIndex(f=>f.getAttribute("aria-selected")==="true"));l[t.key==="ArrowUp"?Math.max(0,g-1):g]?.focus();return}const n=e.closest("[data-order-status-option]");if(n){const o=n.closest("[data-order-status-control]"),c=Array.from(o?.querySelectorAll("[data-order-status-option]")??[]),l=c.indexOf(n);if(t.key==="Escape"){t.preventDefault(),Et(),o?.querySelector("[data-order-status-trigger]")?.focus();return}if(!["ArrowDown","ArrowUp","Home","End"].includes(t.key))return;t.preventDefault();let g=l;t.key==="ArrowDown"&&(g=(l+1)%c.length),t.key==="ArrowUp"&&(g=(l-1+c.length)%c.length),t.key==="Home"&&(g=0),t.key==="End"&&(g=c.length-1),c[g]?.focus();return}const s=e.closest("[data-admin-calendar]");if(s&&t.key==="Escape"){t.preventDefault(),Pe(),s.querySelector("[data-calendar-trigger]")?.focus();return}const i=e.closest("[data-calendar-trigger]");if(i&&t.key==="ArrowDown"){t.preventDefault(),s?.querySelector(".admin-calendar__popover")?.hidden&&i.click(),(s?.querySelector("[data-calendar-date].is-selected")??s?.querySelector("[data-calendar-date].is-today")??s?.querySelector("[data-calendar-date]:not(.is-outside)"))?.focus();return}const r=e.closest("[data-calendar-date]");if(r&&["ArrowLeft","ArrowRight","ArrowUp","ArrowDown"].includes(t.key)){t.preventDefault();const o=Array.from(s?.querySelectorAll("[data-calendar-date]")??[]),c=o.indexOf(r),l=t.key==="ArrowLeft"?-1:t.key==="ArrowRight"?1:t.key==="ArrowUp"?-7:7;o[c+l]?.focus()}});document.addEventListener("input",t=>{const e=t.target;if(e instanceof HTMLInputElement&&e.id==="modal-quantity-input"){ct(Number(e.value));return}if(e instanceof HTMLInputElement&&e.id==="admin-product-search"){ee=e.value,pa();return}if(e instanceof HTMLInputElement&&e.id==="admin-order-search"){ae=e.value,qt();return}if(e instanceof HTMLInputElement&&e.id==="admin-client-search"){ua(e.value);return}if(e instanceof HTMLInputElement&&e.name==="basePrice"&&e.closest("#admin-product-form")){const a=Number(e.value)||0,n={...Vt(),basePrice:a},s=e.closest("form")?.querySelector(".admin-editor-price-preview"),i=s?.querySelector("strong"),r=s?.querySelector("small");i&&(i.textContent=$(D(n,1))),r&&(r.textContent=`опт: ${$(D(n,Q))}`)}});document.addEventListener("submit",t=>{const e=t.target;e instanceof HTMLFormElement&&(e.id==="login-form"?(t.preventDefault(),ue(e)):e.id==="register-form"?(t.preventDefault(),Ge(e)):e.id==="admin-login-form"?(t.preventDefault(),ue(e,!0)):e.id==="admin-product-form"?(t.preventDefault(),fa(e)):e.id==="profile-form"&&(t.preventDefault(),Ye(e)))});document.addEventListener("change",t=>{const e=t.target;if(e instanceof HTMLInputElement&&e.matches("[data-import-products]")){_a(e);return}if(e instanceof HTMLInputElement&&e.matches("[data-import-backup]")){$a(e);return}if(e instanceof HTMLTextAreaElement&&e.dataset.orderNote){const a=B(),n=a.find(s=>s.id===e.dataset.orderNote);n&&(n.managerNote=e.value.trim(),h(u.orders,a),k&&q.updateOrder(n.id,{managerNote:n.managerNote}).then(s=>{h(u.orders,a.map(i=>i.id===s.id?s:i))}).catch(s=>{S=N(s,"Не вдалося зберегти нотатку менеджера."),y()}));return}if(e instanceof HTMLInputElement||e instanceof HTMLSelectElement){if(e instanceof HTMLInputElement&&e.dataset.cartQuantity){xe(e.dataset.cartQuantity,Number(e.value));return}if(e instanceof HTMLInputElement&&e.dataset.partnerToggle){const a=R(),n=a.find(s=>s.id===e.dataset.partnerToggle);if(n){if(n.partner=e.checked,h(u.accounts,a),k){q.updateClient(n.id,{partner:n.partner}).then(s=>{h(u.accounts,a.map(i=>i.id===s.id?s:i)),y()}).catch(s=>{S=N(s,"Не вдалося змінити статус клієнта."),y()});return}y()}return}if(e instanceof HTMLInputElement&&e.dataset.partnerMarkup){const a=R(),n=a.find(s=>s.id===e.dataset.partnerMarkup);if(n){if(n.fixedMarkup=Math.min(.99,Math.max(0,Number(e.value)||0)),h(u.accounts,a),k){q.updateClient(n.id,{fixedMarkup:n.fixedMarkup}).then(s=>{h(u.accounts,a.map(i=>i.id===s.id?s:i)),y()}).catch(s=>{S=N(s,"Не вдалося зберегти персональну ціну."),y()});return}y()}}}});document.querySelector("#product-dialog")?.addEventListener("click",t=>{t.target===t.currentTarget&&(t.currentTarget.close(),at=null)});document.querySelector("#admin-product-dialog")?.addEventListener("click",t=>{t.target===t.currentTarget&&t.currentTarget.close()});document.querySelector("#profile-dialog")?.addEventListener("click",t=>{t.target===t.currentTarget&&t.currentTarget.close()});window.addEventListener("hashchange",zt);async function Aa(){if(k){document.body.dataset.backend="loading";try{F(await q.products()),xt()&&await Ut(),document.body.dataset.backend="online",j(),W(),zt()}catch(t){document.body.dataset.backend="offline",console.error("ToffiPacks backend is unavailable:",t),window.location.hash.startsWith("#admin")&&(S="Сервер тимчасово недоступний. Дані не змінено.",y())}}}U(!0);window.setTimeout(()=>U(!1),460);J();W();zt();qa();Aa();"serviceWorker"in navigator&&window.location.protocol==="https:"&&window.addEventListener("load",()=>{navigator.serviceWorker.register("./sw.js").catch(()=>{})});
